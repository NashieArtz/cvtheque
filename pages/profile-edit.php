...
<script>
    ...
    function addSkillTag(type) {
        ...
        if (val) {
            ...
            tag.innerHTML = `
            <span>${val}</span>
            <span class="skill-tag-remove" onclick="this.parentElement.remove()">✕</span>
        `;
    ...
    function prepareTagsForSubmit() {
        const types = ['hard', 'soft', 'hobbies'];
        types.forEach(type => {
            const list = document.getElementById('list-' + type);
            const hiddenInput = document.getElementById('hidden-' + type);
            const tags = [];

            list.querySelectorAll('.skill-tag-item span:first-child').forEach(span => {
